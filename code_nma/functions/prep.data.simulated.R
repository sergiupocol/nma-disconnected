			  ...
			  sampleSize <- rep(sample.size, n.studies * n.treatments)
			    ...
			    treatmentNumber <- rep(0, n.studies * n.treatments)
			      ...
			      sim.data <- data.frame(studyNumber, treatmentNumber, responders, sampleSize)
			        ...
			        n.to.be.dropped <- n.treatments - 2
			        
			        indices.dropped <- c()
				  for (i in 1:n.studies) {
					      ## For every study I need to randomly drop n.to.be.dropped treatments
					      indices.dropped <- c(indices.dropped,sample(size = n.to.be.dropped, x = ((i - 1) * n.treatments + 1):(i * n.treatments),replace = FALSE))# n... random indices
				    
				  }
				  sim.data <- sim.data[-indices.dropped,]
				    
				    trt.names <- unique(sim.data$treatmentNumber)
